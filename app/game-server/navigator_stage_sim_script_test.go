...
var simScripts = map[string]simScript{
	...
	"303": {
		...
		// **ここが「追いかけダイヤル」という課題の本体**なので、
		// 落ちていたら所見にする (実測 15回中4回で欠落した)。
		MustMention: []string{
			"回し直|追いかけ|追って|ずれたら|変わったら|変わるたび|合わせ直|合わせ換え",
		...
		// 「一致した瞬間に切れ」と言うと、落ち着いて切れば済むところを
		// **急かして焦らせる**。窓を確保した設計の意図そのものを壊す。
		// 「10秒ごとに変わる」は**常識なので言わせない**。説明すると発話が伸び、
		// くどくなる。求めているのは「合わせ直せ」という動作だけ。
		MustNotMention: []string{"瞬間", "一瞬", "10秒ごと", "十秒ごと"},
		// **プレイヤーに謎を解かせない台本にする。**
		// 以前は「タイマーの数字とダイヤルですか」(仕掛けを言い当てる)、
		// 「ダイヤルは5までしかありません」(6-9 で切れない理由を自力で導く)、
		// 「5になったので合わせました」(待つ判断まで自力でやる) と、
		// **ナビが説明しなくても進んでしまう**台本だった。
		// これでは「合わせ直し」を落としても破綻せず、欠落を見逃す。
