...
`game-server/proto/` 以下の `.pb.go` は Docker ビルド時に自動生成されます。
ローカルビルドでは手動で生成が必要です。
...
```bash
PKG_CONFIG_PATH="/opt/local/lib/pkgconfig" go build -o game-server .
./game-server config.toml
```
