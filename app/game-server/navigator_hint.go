...
type StageProgress struct {
	...
	//
	// 観察系のステージ (104 早い者勝ち・207 息が合わない など) は、
...
`
	...
	case HintL4:
		...
		//
		// 以前は「正解文に『これは言わない』と但し書きがあれば優先」という
		// **散文の指示**で守らせていたが、L4 では正解文が生の色名込みで載り、
		// ここが「そのまま伝えてください」と指示するため**但し書きが負ける**
		// (目の前にある語はなぞられる — N-1)。
		// `keep_cut_secret` で分岐し、**そもそも「そのまま伝えろ」と言わない**。
		if stage.KeepCutSecret {
			return base + "## 現在の許可ヒントレベル: L4 (直言) — ただし**切る線の色名は除く**\n" +
				"手順・数字・場所は明確に伝えてよい段階です。\n" +
				"**ただしこの課題では、切る線の色名だけは最後まで言いません。**" +
				"色名を言うと課題そのものが成立しなくなるためです。" +
				"色を確定させるのはプレイヤーの仕事で、あなたはそこへ導いてください。\n" +
