...
| 12 | ストリーム待ち時間を5秒へ(実機確認時) | ナビゲーターの音声が途中で途切れた。当時 bridge は後続音声を **2秒**しか待たずに打ち切っており、TTS が通常2〜3秒かかるため**わずかな遅れでも音声が千切れる**設定だった。待ち時間を **5秒**へ伸ばして解決。長くすると遅延中ずっと PTT を握って無線を塞ぐため、大半のケースを救えて塞ぎすぎない範囲に留めた。**遅延そのものの主因は TTS プロンプトの表情タグ**で、そちらは廃止済み (`navigator_design.md` §4.1)。**この待ち合わせは決定16で不要になり削除した** |
| 13 | ナビゲーター発話を1パケットにまとめる(実機確認時) | TTS の1チャンクが **58.97秒**かかり、先に完成していた後続チャンクが1分近く滞留して発話全体が遅れた。当時は発話を句点で分割して順次送っており、「先頭ができ次第すぐ流せる」利点の裏返しで**1チャンクの遅延が発話全体を人質に取る**状態だった。全 PCM が揃うのを待ってから連結し、**1回だけ Opus エンコードして1パケットで送る**方式に変更。レイテンシは「最も遅いチャンク」に律速されるぶん増えるが、出るときには必ず発話が揃っている。単一の Ogg ストリームになるため、bridge 側で別々の Ogg を繋ぐ必要もなくなった (`navigator_decisions.md` 決定12・18) |
| 14 | 分割送信の仕組みを撤去 | 決定13で発話が1パケットになり、混線・効果音も元から単発だったため、**分割送信の経路は実際には使われなくなっていた**。使われないまま残すと誤って復活させる余地が残り、bridge 側の待ち合わせロジック (決定12) も読み手を惑わせる。proto の `StreamStatus` / `stream_id` を削除して `AudioChunk` を `ogg_opus_data` のみにし、radio-bridge の stream 単位キュー・後続待ち・クローズ済み管理、emulator の同等ロジックも撤去した。**1 パケット = 1 再生サイクル**に一本化されている。再び分割が必要になったら proto から作り直す
