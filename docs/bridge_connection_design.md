...
現状は wl-game-server が gRPC クライアントとして、config の単一 `host:port` に
ダイヤルして radio-bridge(gRPCサーバー)と1本の双方向ストリームを張っている。
...
- **再接続**: bridge 側が再接続ループを持つ(現行 `bridge_client.go` の
  リトライロジックを Rust 側へ移植)。
...
| radio-bridge-test-client (Go) | bridge へダイヤルする現方式が成立しなくなる。wl-game-server 相当のモックサーバーを内蔵するか、wl-game-server 経由の試験に切り替える |
...
6. radio-bridge-test-client の試験方式見直し
