...
で動かす(`operation_flow.md` §8)。プロセスごとに異なる値は環境変数で与え、
`config.toml` は全プロセスで共有できるようにする(bind mount したまま
`environment` だけ変えてサービスを増やせる)。
...
- **いずれの経路でも未設定の場合は起動時にエラーで停止**する。
  ID 未設定は接続拒否、デバイス未設定は無音という分かりにくい失敗になるため、
  起動時点で原因を明示して落とす。
