...
| IDは詰めて振る | 有効は 101-105 / 201-208 / 301-305。無効化したら同じ帯の90番台へ退避する | S-9 |
...
| 押し切ったら答えを出す | 103 は押下中2色見せ、押し切ると切る線だけ点灯。最初から両方見せないとひねりが消える | C-13 |
...
- **紙資料**は未制作。**A4裏表1枚**に資料1〜資料4を印刷する設計
  (資料1=モールス対照表 / 資料2=記号 / 資料3=数字 / 資料4=回路図)。
  資料4は **X/Y の2系統・計10端子**を描く。あわせて Core 本体へ
  **シリアル銘板(CoreID記載)**を貼る — 音声申告で使う CoreID と**同じ1枚**で、
  下1桁の奇数/偶数が 205 の端子系統を決める(採番で奇数偶数を混ぜる)。
  `config.toml` の `[mission_sheet]` は印刷物の実測値なので、
