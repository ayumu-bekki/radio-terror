...
| 206 は直言しない | 正解がプレイヤーの記憶にしかない。`l4_pct = 0` で L4 を塞ぐ。`StageHintOverride` を `int` に戻すと復活する | N-36 |
| 但し書きは L4 で負ける | 「言うな」と answer に書いても L4 は伏せ字が外れる。205・202・203 は `keep_cut_secret` で機械的に伏せる | N-38 |
| 直言はイージーだけ | ノーマル・ハードは `l4_pct = 0`。ナビは答えそのものを言わない。206 のステージ単位指定は別途残す | N-39 |
...
| 205 は両系統を言う | 端子は X/Y の2系統。「奇数ならX4、偶数ならY2」と並べる。サーバーはシリアルを知らない | D-6 |
| 202 の表は実装に1箇所 | 対照表は `scenario_codebook.go` だけ。config には出さない。紙との一致はテストで担保する | D-3 |
| 202 は cut から逆引き | 表示を先に抽選すると使用済みの色を避けられず S-2 を破る。全消灯は故障と紛れるので載せない | D-7 |
| 資料の見出しは色名 | 「LED2〜LED5」と位置で書かない。実機のランプは色で識別する。ロータリーは0-5なので頭文字区間は 5・4・4・4・4・5 | D-8 |
| モールスに数字を使わない | 数字は全て5要素で数え違えると隣になる。211 は英字1文字。資料1に数字は載せない | N-41 |
